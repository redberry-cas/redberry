...
public class BruteForcePermutationIteratorTest {
    ...
    public void testInfLoop() {
        ...
        for (int tt = 0; tt < 1000; ++tt) {
            ...
            for (int i = 0; i < 10; ++i) {
                Permutation p = null;
                try {
                    p = new Permutation(i % 2 == 0 ? true : false, Combinatorics.randomPermutation(10));
                } catch (Exception e) {
                }
                if (p != null)
                    permutations.add(p);
            ...
            try {
                ...
                for (Permutation p : permutations) {
                    ...
                    s = "permutations.add ( new Permutation(" + p.sign + ", " + s + "));";
    ...
    public void testInfLoop2() {
        List<Permutation> permutations = new ArrayList<>();
        permutations.add(new Permutation(true, 1, 3, 9, 4, 7, 6, 2, 0, 8, 5));
        permutations.add(new Permutation(false, 3, 8, 4, 6, 9, 1, 2, 0, 5, 7));
        permutations.add(new Permutation(false, 9, 1, 6, 3, 8, 2, 4, 5, 7, 0));
        permutations.add(new Permutation(false, 5, 7, 8, 1, 9, 6, 4, 3, 0, 2));
        permutations.add(new Permutation(true, 5, 9, 3, 0, 8, 4, 1, 6, 7, 2));
        permutations.add(new Permutation(false, 7, 0, 6, 2, 1, 3, 9, 4, 5, 8));
        permutations.add(new Permutation(false, 0, 5, 8, 6, 7, 2, 9, 4, 1, 3));
