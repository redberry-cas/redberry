...
public final class Context2 {
    ...
    Context2(ContextConfiguration cc) {
     ...
     */
    public VarDescriptor getNameDescriptor(int nameId) {
    ...
    public final class GlobalTensors {
        ...
        /* Initializing stuff with metric tensors */ {
            ...
            for (IndexType mt : metricTypes) {
                ...
                final VarDescriptor nd = nameManager.resolveVarOrFunction(
                        contextConfiguration.metricName, st,
                        new VarIndicesProvider.SymmetriesHolder(IndicesSymmetries.create(st)),
                        false
                );
        ...
        public SimpleTensor createKronecker(int index1, int index2) {
            ...
            if (!Context2.this.isMetric(type) && IndicesUtils.getState(index2)) {
                int t = index1;
                index1 = index2;
                index2 = t;
            }
         ...
         * DiracDelta
         */
        private final VarDescriptor DiracDelta;

        /* Initializing stuff with DiracDelta */ {
            DiracDelta = nameManager.resolveVarOrFunction("DiracDelta",
                    StructureOfIndices.getEmpty(),
                    new VarIndicesProvider.SymmetriesHolder(IndicesSymmetries.getEmpty()),
                    true);
        }
        ...
        public TensorField createDeltaFunction(Tensor a, Tensor b) {
            return Tensors.field(DiracDelta.id, IndicesFactory.EMPTY_SIMPLE_INDICES, new Tensor[]{a, b});
        }
