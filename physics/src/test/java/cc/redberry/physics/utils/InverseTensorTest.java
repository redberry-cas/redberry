...
public class InverseTensorTest {
    ...
    @Test
    public void testVectorField1() throws Exception {
    ...
    @Test
    public void testVectorField1a() throws Exception {
        Transformation[] transformations = new Transformation[]{Tensors.parseExpression("k_a*k^a=1")};
    ...
    @Test
    public void testVectorField2() throws Exception {
        Expression toInverse = Tensors.parseExpression("D_mn = k_m*k_n-(1/a)*k_i*k^i*g_mn");
        Expression equation = Tensors.parseExpression("D_ab*K^ac=d_b^c");
        Tensor[] samples = {Tensors.parse("g_mn"), Tensors.parse("g^mn"), Tensors.parse("d_m^n"), Tensors.parse("k_m"), Tensors.parse("k^b")};
        ...
        Tensor actual = null;
        if (mapleBinDir != null) {
            actual = InverseTensor.findInverseWithMaple(toInverse, equation, samples, false, new Transformation[0], mapleBinDir, temporaryDir);
            Assert.assertTrue(TensorUtils.equals(expected, actual));
        }
        if (mathematicaBinDir != null) {
            actual = InverseTensor.findInverseWithMathematica(toInverse, equation, samples, false, new Transformation[0], mathematicaBinDir, temporaryDir);
            Assert.assertTrue(TensorUtils.equals(expected, actual));
        }
    }
    ...
    public void test2() throws Exception {
        ...
        if (mathematicaBinDir != null) {
            ...
            temp = new TransformationCollection(transformations).transform(temp);
            temp = FactorTransformation.factor(temp);

            Assert.assertTrue(((Expression) temp).isIdentity());
        }
    }
    ...
    public void test3() throws Exception {
        ...
        Tensor actual;
        if (mapleBinDir != null) {
            actual = InverseTensor.findInverseWithMaple(
                    toInverse,
                    equation,
                    samples,
                    false,
        ...
        if (mathematicaBinDir != null) {
            actual = InverseTensor.findInverseWithMathematica(
                    toInverse,
                    equation,
                    samples,
                    false,
