 ...
 * mode params in next().
 */
class TreeIteratorImpl implements TreeIterator {

    private final TreeTraverseIterator iterator;
    private final TraverseState state;

    TreeIteratorImpl(Tensor tensor, TraverseGuide guide, TraverseState state) {
        this.iterator = new TreeTraverseIterator(tensor, guide);
        this.state = state;
    }

    TreeIteratorImpl(Tensor tensor, TraverseState state) {
