 ...
 * acting on tensor indices, can change sign of tensor, i.e. {@code A_{mn} = -A_{nm}}, 
 * and can not - {@code g_{mn} = g_{nm}}. For more information see methods summary.
 * 
 * @see Permutation
 * @see Indices
 * @see Tensor
...
public class Symmetry extends Permutation {
     ...
     * <i>one-line</i> notation and {@code signum} - property of permutation.
     * If {@code signum == false}, it means that this permutation does not change 
     * object, when acting on it. If {@code signum == true}, it means that 
     * permutation changes sign of object, when acting on it, i.e. this is 
     * antisymmetry.
     * 
     * @param permutation <i>one-line</i> notated integer array, representing 
     * a permutation.
    ...
    public Symmetry inverse() {
        if (inverse == null)
            calculateInverse();
        return new Symmetry(inverse, signum);
    }
